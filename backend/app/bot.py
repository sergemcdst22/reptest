...
async def approve_order(query: types.CallbackQuery):
    ...
    await query.answer()
    await query.message.edit_text(f"Спасибо, заказ ({txt_order}) сохранен. Переведите его цену: {f'{price:,}.00'} на карту {card_number}, в комментарии укажите номер заказа: {str(order_id)[-4:]}. Если перевод не будет доставлен до {str(removing_at)[:-6]}, заказ будет удален")
    await bot.send_message(admin_id, f"Заказ от пользователя {query.from_user.full_name} (tg id: {query.from_user.id}) номер {str(order_id)[-4:]} на сумму {f'{price:,}.00'}. Подробно о заказе: {txt_order}")
